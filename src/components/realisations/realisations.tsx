...
class Realisations extends Component {
	...
	private hasProjectPictures = false;
	 ...
	 * @param scroll scroll to the description container element
	 */
	private loadProject: (project: Project, event: any) => void = (project: Project, event: any) => {
		this.projectPictures = []
		...
		// scroll the the project
		let descriptionContainerElement = document.getElementById("descriptionContainer");
		if (descriptionContainerElement && event)
			descriptionContainerElement.scrollIntoView();
	}
	...
	private handleSectionClick: (id: string) => void = (id: string) => {
		...
		if (section) {
			...
			this.loadProject(section.projects[0], false);
		}
	...
	render() {
		return (
			<div>
				...
				<div id="pictures" className={this.state.currentPictures.length > 3 ? "shake" : ""}>
